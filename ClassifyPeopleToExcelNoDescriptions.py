# 2018 02 11 John Blue this is Python 3
# 2018 02 15 John Blue Added remove punctuation from target string code
#            terms like " corn " were lost in " corn," 
...
# The input file must have Twitter screen_name in column 2, Twitter description in column 5, 
# and source file name or ID in column 26. 
# The source file originates from audiense.com, and then the column 26 source name/ID was added.

# use
# python ClassifyPeopleToExcelNoDescriptions.py new_excel-file.xlsx ag-terms.txt
...
out_acol = 1
print('{}|{}|'.format("screen_name", "source"), end=" ")
...
translator = str.maketrans('', '', string.punctuation) # to remove punctuation in target string
...
# 2018 02 11 changed .encode('utf-8').lower() to .lower()
    targetstring = (
        row_screen_name +
        " " +
        row_description).lower()
    
    targetstring=targetstring.translate(translator)

# output initial info on the account
    print('{}|{}|'.format(row_screen_name, row_source), end=" ")
    ...
    for t in terms:  # loop through terms
        ...
        if t in targetstring:
            print('{}|'.format('1'), end=" ") #use 1 instead of t to get a numeric value in worksheet
